...
struct allocator_c11 {// Тип данных, который может хранить аллокатор
    using value_type = T;
    ...
    allocator_c11 () : current_block_size(BlockSize), allocated_elements(0),
    ...
    template <class U> allocator_c11 (const allocator_c11<U>&) noexcept {}
...
constexpr bool operator== (const allocator_c11<T>& a1, const allocator_c11<U>& a2) noexcept {
    return true;
}
...
// Allocator - тип аллокатора (по умолчанию allocator_c11<T>)
template <typename T, size_t MaxSize, typename Allocator = allocator_c11<T>>
...
// template <typename T, typename Allocator = allocator_c11<T>>
...
int main() {
    ...
    // 3) Создание экземпляра std::map<int, int> c новым аллокатором
    std::map<int, int, std::less<int>, allocator_c11<std::pair<const int, int>, 10>> map2;
    ...
    // 3) Создание экземпляра LimitedContainer c новым аллокатором, ограниченным 10 элементами
    LimitedContainer<std::pair<const int, int>, 10, allocator_c11<std::pair<const int, int>, 10>> limited_map; 
    ...
    // // Создаем список c использованием `allocator_c11`
    // DoubleLinkedList<int,allocator_c11<int>> list;
